...
```{r, echo=TRUE}
packages = c('tidyverse','readxl','forcats','tidyr','gridExtra','scales')
...
```
# 1. Task 1: Pareto Chart for Returns by Sub-Category of Products
## 1.1 Challenges
...
### 1.3.2 Join Returns and Order Sheets to obtain Detailed list of Returned Orders
```{r, echo=TRUE}
...
### 1.3.3 Count Returned Orders by Sub-Category and Sort Sub-Categories in Descending Order
```{r, echo=TRUE}
...
### 1.3.4 Compute Cumulative Frequency and express as Percentage
```{r, echo=TRUE}
...
```
# 2. Task 2: Population Age-Sex Pyramid
## 2.1 Challenges
...
### 2.3.2 Extract Males Data
```{r, echo=TRUE}
males <- population %>% filter(population$Sex=="Males")
males <- as.data.frame(males)
```

### 2.3.3 Group and Summarise Males Data by Age Group
```{r, echo=TRUE}
males <- males %>% group_by(AG) %>% summarise(cover = sum(Pop))
```

### 2.3.4 Extract Starting Numeric Value of Males Age Group for Sorting
```{r, echo=TRUE}
...
### 2.3.5 Extract Females Data
```{r, echo=TRUE}
females <- population %>% filter(population$Sex=="Females")
females <- as.data.frame(females)
```

### 2.3.6 Group and Summarise Females Data by Age Group
```{r, echo=TRUE}
females <- females %>% group_by(AG) %>% summarize(cover = sum(Pop))
```

### 2.3.7 Extract Starting Numeric Value of Females Age Group for Sorting
```{r, echo=TRUE}
...
### 2.3.8 Combine Males and Females Age Band Summaries into 1 Table
```{r, echo=TRUE}
MF <- rbind(males,females)
```

### 2.3.9 Plot Visualisation
```{r, echo=TRUE}
